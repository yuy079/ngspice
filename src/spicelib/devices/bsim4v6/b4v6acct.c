...
int
BSIM4v6accept(CKTcircuit *ckt, GENmodel *inModel)
{
            ...
            if (vgs > model->BSIM4v6vgsMax)
                if (warns_vgs < maxwarns_vgs) {
                    printf("Instance: %s Model: %s Time: %g |Vgs|=%g has exceeded Vgs_max=%g\n",
                           here->BSIM4v6name, model->BSIM4v6modName, ckt->CKTtime, vgs, model->BSIM4v6vgsMax);
                    if (slogp)
            ...
            if (vgd > model->BSIM4v6vgdMax)
                if (warns_vgd < maxwarns_vgd) {
                    printf("Instance: %s Model: %s Time: %g |Vgd|=%g has exceeded Vgd_max=%g\n",
                           here->BSIM4v6name, model->BSIM4v6modName, ckt->CKTtime, vgd, model->BSIM4v6vgdMax);
                    if (slogp)
            ...
            if (vgb > model->BSIM4v6vgbMax)
                if (warns_vgb < maxwarns_vgb) {
                    printf("Instance: %s Model: %s Time: %g |Vgb|=%g has exceeded Vgb_max=%g\n",
                           here->BSIM4v6name, model->BSIM4v6modName, ckt->CKTtime, vgb, model->BSIM4v6vgbMax);
                    if (slogp)
            ...
            if (vds > model->BSIM4v6vdsMax)
                if (warns_vds < maxwarns_vds) {
                    printf("Instance: %s Model: %s Time: %g |Vds|=%g has exceeded Vds_max=%g\n",
                           here->BSIM4v6name, model->BSIM4v6modName, ckt->CKTtime, vds, model->BSIM4v6vdsMax);
                    if (slogp)
            ...
            if (vbs > model->BSIM4v6vbsMax)
                if (warns_vbs < maxwarns_vbs) {
                    printf("Instance: %s Model: %s Time: %g |Vbs|=%g has exceeded Vbs_max=%g\n",
                           here->BSIM4v6name, model->BSIM4v6modName, ckt->CKTtime, vbs, model->BSIM4v6vbsMax);
                    if (slogp)
            ...
            if (vbd > model->BSIM4v6vbdMax)
                if (warns_vbd < maxwarns_vbd) {
                    printf("Instance: %s Model: %s Time: %g |Vbd|=%g has exceeded Vbd_max=%g\n",
                           here->BSIM4v6name, model->BSIM4v6modName, ckt->CKTtime, vbd, model->BSIM4v6vbdMax);
                    if (slogp)
