...
*/
{
    ...
    for (;;) {
        ...
        if (!ciprefix("write", buffer) &&
            !ciprefix(".lib", buffer)) {    // exclude 'write' command so filename case preserved
            char *s;
            for (s = buffer; *s && (*s != '\n'); s++)
                *s = (char) tolower(*s);
            if (!*s) {
        ...
        if (ciprefix(".lib", line)) {
            ...
            if (*y) {
                ...
                *z = '\0';
                ...
                if (lib_idx < 0) {
                    fprintf(stderr, "ouch, library file %s is missing\n", s);
                    controlled_exit(EXIT_FAILURE);
                }
                ...
                if (!section_def) {
                    fprintf(stderr, "ouch, library file %s, section definition %s is missing\n", s, y);
                    controlled_exit(EXIT_FAILURE);
                }
                ...
                /* insert the library section definition into `c' */
                {
                    ...
                    for (; t; t=t->li_next)
                        if(ciprefix(".endl", t->li_line))
                            break;
                    if (!t) {
                        fprintf(stderr, "ouch, .endl not found\n");
                        controlled_exit(EXIT_FAILURE);
                    }
                    t->li_line[0] = '*';
                    t->li_line[1] = '>';
