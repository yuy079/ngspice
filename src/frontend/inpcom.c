...
static char*
search_parameter(char *str, struct function *fcn, int *which, char *str_begin)
{
...
inp_do_macro_param_replace(struct function *fcn, char *params[])
{
    ...
    while ((param_ptr = search_parameter(param_ptr, fcn, &i, str)) != NULL) {
        char *p;
        int is_vi = 0;
...

        {
            ...
            int prefix_len = (int) (param_ptr - rest);
